...
def getimgtable(imgpath):  #把黑白图片变成0或1的数阵图像
    img = Image.open(imgpath)
    table = []
    pixdata = img.load()
    w, h = img.size
...
def scissor(imgpath):
    """实现剪切图片"""
    center = [16, 29, 43, 59]  #验证码图片4个数字的重心
...
def martixtoline(filename):
    ...
    f = open(filename)
    for i in range(24):
        linestr = f.readline() # 一行一行读文件
        for j in range(14):
            line[0, 14 * i + j] = int(linestr[j])  #把数阵文件中每一行拼接起来，成为一个行向量
    return line
...
def classify0(inX, dataSet, labels, k):  # KNN算法函数
    ...
    diffMat = tile(inX, (dataSetSize, 1)) - dataSet  #每个维度作差
    sqDiffMat = diffMat ** 2   #每个维度平方运算
    sqDistances = sqDiffMat.sum(axis=1)     #每个维度相加
    distances = sqDistances ** 0.5   #开方
    sortedDistIndicies = distances.argsort()   #排序，得到的是原来位置的索引
    classCount = {}
    for i in range(k):
        voteIlabel = labels[sortedDistIndicies[i]]   #得到排名靠前的K个标签
        classCount[voteIlabel] = classCount.get(voteIlabel, 0) + 1   #统计排名靠前的K个标签中，那个标签次数出现最多
    sortedClassCount = sorted(classCount.items(), key=operator.itemgetter(1), reverse=True)
    return sortedClassCount[0][0]    #返回排名靠前的K个标签中，标签次数出现最多的标签
...
def login(username):
    ...
    print('验证码已写入到本地！')
    checkcode = ''
    bit = biting('checkcode.gif', 88)  #88是阀值，去除验证码干扰线和随机噪声点
    bit.save('checkcode.bmp')
    scissor('checkcode.bmp')
    for x in range(4):
        t = getimgtable('num/checkcode-' + str(x) + '.bmp')
...
def boom(start,scope):  #start是起始学号如1700000，scope是起始学号往后的范围
